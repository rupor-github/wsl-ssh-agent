...
// Loads an icon from resource and shows it in tray.
// Shell_NotifyIcon: https://msdn.microsoft.com/en-us/library/windows/desktop/bb762159(v=vs.85).aspx
func (t *winTray) setIconFromResource(resID uint32) error {
	const NIF_ICON = 0x00000002

	h, err := t.loadIconFromResource(resID)
	if err != nil {
		return err
	}
...
func (t *winTray) wndProc(hWnd windows.Handle, message uint32, wParam, lParam uintptr) (lResult uintptr) {
	...
	case WM_CREATE:
		...
		registered, _, _ := pRegisterSessionNotification.Call(
			uintptr(hWnd),
...
func (t *winTray) hideMenuItem(menuItemId, parentId uint32) error {
	...
	if res == 0 && !errors.Is(err, windows.ERROR_SUCCESS) {
		return err
	}
	t.delFromVisibleItems(parentId, menuItemId)
...
func (t *winTray) loadIconFromResource(resID uint32) (windows.Handle, error) {
	const IMAGE_ICON = 1              // Loads an icon
	const LR_DEFAULTSIZE = 0x00000040 // Loads default-size icon for windows(SM_CXICON x SM_CYICON) if cx, cy are set to zero
	const LR_SHARED = 0x00008000      // Shares the image handle if the image is loaded multiple times

	res, _, err := pLoadImage.Call(
		uintptr(unsafe.Pointer(t.instance)),
		uintptr(resID),
		IMAGE_ICON,
		0,
		0,
...
// MAKEINTRESOURCE https://docs.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-makeintresourcea
func MakeIntResource(i int16) []byte {
	var b = []byte{0, 0, 0, 0}
	binary.LittleEndian.PutUint16(b[0:], uint16(i))
	return b
}
...
func SetIcon(iconBytes []byte) {
	if resID := IntResource(iconBytes); resID != 0 {
		if err := wt.setIconFromResource(resID); err != nil {
			log.Printf("Unable to set icon from resource: %v", err)
			return
		}
	} else {
...
func (item *MenuItem) SetIcon(iconBytes []byte) {
	...
	if resID := IntResource(iconBytes); resID != 0 {
		h, err = wt.loadIconFromResource(resID)
		if err != nil {
			log.Printf("Unable to load icon from resource: %v", err)
			return
		}
	} else {
		iconFilePath, err := iconBytesToFilePath(iconBytes)
